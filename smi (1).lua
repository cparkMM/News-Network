...
function main()
	...
	autoupdate("https://raw.githubusercontent.com/cparkMM/News-Network/master/upd.php", '['..string.upper(thisScript().name)..']: ', "если, пошло что-то не по плану, пишите в ВК: kirill_batozhok")
		...
		for line in text: gmatch("[^\n]+") do
			...
			elseif line:find('Должность:%s+%{0099ff%}(.*) %/') then
				...
				elseif my['rc'] == 'Радиоцентр ЛВ' then
					my['edit_teg'] = 'LV-N | '
				elseif my['rc'] == 'Управляющий СМИ' then
					my['edit_teg'] = 'MM | '
			  end
			end
		end
...
function sampev.onServerMessage(color,text)
	if text:find('Добро пожаловать на Diamond Role Play!') and check_my then
		sampSendChat('/stats')
		sampAddChatMessage('{ffffff}•{ffcb00} [Подсказка]{ffffff} Автообновление скрипта, временно не работает!')
	end
end
function imgui.OnDrawFrame()
	...
	if window_binder.v then
	 ...
	 imgui.Begin(u8'News Network | Binder',window_binder, imgui.WindowFlags.ShowBorders+imgui.WindowFlags.NoCollapse+imgui.WindowFlags.NoResize)
	 imgui.Columns(2,nil,false)
...
function cmd_sobes(id)
	if #id>0 then
		lua_thread.create(function()
			sampSendChat('Здравствуйте, Вы попали на собеседование в '..my['rc']..('.'))
			wait(2000)
			sampSendChat('Предъявите, пожалуйста, Ваши документы, а именно: паспорт, лицензии и мед.карту.')
			wait(2000)
			...
			if isKeyJustPressed(VK_Y) then
				...
				local random1 =
				{
					'Расшифруйте аббревиатуру "СМИ"',
					'В каком Радиоцентре мы сейчас находимся?',
					'Чем занимается человек, с должностью "Режиссёр"?'
				}
				sampSendChat(random1[math.random(0,#random1)])
				wait(300)
				sampAddChatMessage('{ffffff}•{ffcb00} [Подсказка]{ffffff} Нажмите {00ff00}Y{ffffff} - если всё хорошо и {ff0000}N{ffffff} - если всё плохо', -1)
				while not isKeyJustPressed(VK_Y) and not isKeyJustPressed(VK_N) do wait(0) end
				if isKeyJustPressed(VK_Y) then
					...
					while not isKeyJustPressed(VK_Y) and not isKeyJustPressed(VK_N) do wait(0) end
					if isKeyJustPressed(VK_Y) then
						sampSendChat('Поздравляю, Вы прошли собеседование в наш Радиоцентр!')
					wait(2000)
					sampSendChat('/r'..u8:decode((input_prefix_r.v and input_prefix_r.v or ''))..' Человек с номером в паспорте: '..id..', прошёл собеседование!')
						cmd_invite(id)
...
function cmd_uninvite(arg)
	...
	if tonumber(my['rang']) < 8 then sampAddChatMessage('{ffffff}•{ffcb00} [Подсказка]{ffffff} Вам ещё не доступна данная функция', 0xffffff)
	elseif arg1 == nil or arg1 == "" or arg2 == nil or arg2 == "" then sampAddChatMessage('{ffffff}•{ffcb00} [Подсказка]{ffffff} Введите команду, обязательно указав id игрока и причину без тэга (/uninvite id причина)', 0xffffff)
	else
	 lua_thread.create(function()
...
function cmd_invite(arg)
	if tonumber(my['rang']) < 9 then sampAddChatMessage('{ffffff}•{ffcb00} [Подсказка]{ffffff} Вам ещё не доступна данная функция', 0xffffff)
	elseif arg:match("%d+") ~= arg then sampAddChatMessage('{ffffff}•{ffcb00} [Подсказка]{ffffff} Введите команду, обязательно указав id игрока (/invite id)', 0xffffff)
	else
 	lua_thread.create(function()
...
function cmd_rang(arg)
	if tonumber(my['rang']) < 9 then sampAddChatMessage('{ffffff}•{ffcb00} [Подсказка]{ffffff} Вам ещё не доступна данная функция', 0xffffff)
	elseif arg:match("%d+") ~= arg then sampAddChatMessage('{ffffff}•{ffcb00} [Подсказка]{ffffff} Введите команду, обязательно указав id игрока (/rang id)', 0xffffff)
	else
 	lua_thread.create(function()
		sampSendChat('/do В левой руке кейс.')
		wait(2000)
...
function cmd_fwarn(arg)
	...
	if tonumber(my['rang']) < 8 then sampAddChatMessage('{ffffff}•{ffcb00} [Подсказка]{ffffff} Вам ещё не доступна данная функция', 0xffffff)
	elseif arg1 == nil or arg1 == "" or arg2 == nil or arg2 == "" then sampAddChatMessage('{ffffff}•{ffcb00} [Подсказка]{ffffff} Введите команду, обязательно указав id игрока и причину без тэга (/fwarn id причина)', 0xffffff)
	else
		lua_thread.create(function()
		sampSendChat('/do В руках планшет с логотипом организации.')
		wait(2000)
...
function cmd_unfwarn(arg)
	...
	if tonumber(my['rang']) < 8 then sampAddChatMessage('{ffffff}•{ffcb00} [Подсказка]{ffffff} Вам ещё не доступна данная функция', 0xffffff)
	elseif arg1 == nil or arg1 == "" or arg2 == nil or arg2 == "" then sampAddChatMessage('{ffffff}•{ffcb00} [Подсказка]{ffffff} Введите команду, обязательно указав id игрока и причину без тэга (/unfwarn id причина)', 0xffffff)
	else
		lua_thread.create(function()
		sampSendChat('/do В руках планшет с логотипом организации.')
		wait(2000)
...
function cmd_black(arg)
	if tonumber(my['rang']) < 11 then sampAddChatMessage('{ffffff}•{ffcb00} [Подсказка]{ffffff} Вам ещё не доступна данная функция', 0xffffff)
	elseif arg:match("%d+") ~= arg then sampAddChatMessage('{ffffff}•{ffcb00} [Подсказка]{ffffff} Введите команду, обязательно указав id игрока (/black id)', 0xffffff)
	else 
		lua_thread.create(function()
			sampSendChat('/do В руках планшет с логотипом организации.')
			wait(2000)
...
function cmd_offblack(arg)
	if tonumber(my['rang']) < 11 then sampAddChatMessage('{ffffff}•{ffcb00} [Подсказка]{ffffff} Вам ещё не доступна данная функция', 0xffffff)
	elseif arg:match("%d+") ~= arg then sampAddChatMessage('{ffffff}•{ffcb00} [Подсказка]{ffffff} Введите команду, обязательно указав id игрока (/offblack id)', 0xffffff)
	else 
		lua_thread.create(function()
			sampSendChat('/do В руках планшет с логотипом организации.')
			wait(2000)
...
function cmd_setskin(arg)
	if tonumber(my['rang']) < 8 then sampAddChatMessage('{ffffff}•{ffcb00} [Подсказка]{ffffff} Вам ещё не доступна данная функция', 0xffffff)
	elseif arg:match("%d+") ~= arg then sampAddChatMessage('{ffffff}•{ffcb00} [Подсказка]{ffffff} Введите команду, обязательно указав id игрока (/setskin id)', 0xffffff)
	else
		lua_thread.create(function()
...
function checkUpdate()
   ...
   postrequest.headers = {
	...
	asyncHttpRequest("POST", "https://raw.githubusercontent.com/cparkMM/News-Network/master/upd.php", postrequest,
		function(response)
         if updateInfo ~= nil then
            ...
            if tonumber(updateInfo['version_number']) ~= thisScript().version_num then
               message("Была найдена новая версия: "..updateInfo['version']..'. Начинаю установку..')
